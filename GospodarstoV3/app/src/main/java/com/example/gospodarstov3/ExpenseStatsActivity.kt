...
class ExpenseStatsActivity: AppCompatActivity() {
    ...
    fun getPieChartArrays()
    {
        ...
        val sourceArray=resources.getStringArray(R.array.Category)

        val valueArray = IntArray(9){0}
        var i = 0
        for(s in sourceArray)
        {
            valueArray[i]=databaseHandler.getSumExpenseByCategory(s)
            i++
        }
    ...
    private fun populatePieChart(values: IntArray, labels: Array<String>) {
        ...
        pieShades.add(Color.parseColor("#362511"))
        pieShades.add(Color.parseColor("#EE964D"))
